...
function SetupNavigationBar(handle)
    ...
    set(t,'string',sprintf('ACQNUM: %d',myScriptData.ACQNUM));
...
function InitFiducials(handle,mode)
    ...
    FIDSDISPLAY.fidslist = {'P-wave','QRS-complex','T-wave','QRS-peak','T-peak','Baseline','Activation','Recovery','Reference','Fbase'};     
    events.num = [1:10];
    if FIDSDISPLAY.MODE == 2,
        FIDSDISPLAY.fidslist = {'Baseline','Fbase'};
        events.num = [6 10];
    end
    ...
    events.maxn = length(myScriptData.GROUPLEADS);
...
function InitDisplayButtons(handle),
    ...
    group = myScriptData.GROUPNAME;
    if (isempty(myScriptData.DISPLAYGROUPF))|(myScriptData.DISPLAYGROUPF == 0),
        myScriptData.DISPLAYGROUPF = 1:length(group);
    end
    myScriptData.DISPLAYGROUPF = intersect(myScriptData.DISPLAYGROUPF,[1:length(group)]);
...
%    if isempty(FIDSDISPLAY.XWIN);
    ...
    switch myScriptData.DISPLAYTYPEF,
        ...
        case 2,
            ...
            for p=1:numgroups, 
                leads = myScriptData.GROUPLEADS{groups(p)};
                ...
                FIDSDISPLAY.NAME{p} = [myScriptData.GROUPNAME{groups(p)} ' RMS']; 
            end
        ...
        case 3,
            ...
            ch  = []; 
            for p=groups, 
                ch = [ch myScriptData.GROUPLEADS{p}]; 
                FIDSDISPLAY.GROUP = [FIDSDISPLAY.GROUP p*ones(1,length(myScriptData.GROUPLEADS{p}))];
                FIDSDISPLAY.LEADGROUP = [FIDSDISPLAY.GROUP myScriptData.GROUPLEADS{p}];
                FIDSDISPLAY.LEAD = [FIDSDISPLAY.LEAD myScriptData.GROUPLEADS{p}];
                for q=1:length(myScriptData.GROUPLEADS{p}), FIDSDISPLAY.NAME{end+1} = sprintf('%s # %d',myScriptData.GROUPNAME{p},q); end 
            end
            for p=1:length(groups),
                FIDSDISPLAY.GROUPNAME{p} = [myScriptData.GROUPNAME{groups(p)}]; 
            end 
...
function ButtonUp(handle)
   ...
   % looping is activated)
    global FIDSDISPLAY TS myScriptData;
    events = FIDSDISPLAY.EVENTS{FIDSDISPLAY.SELFIDS};  
    if events.sel(1) > 0,
 	    point = get(FIDSDISPLAY.AXES,'CurrentPoint');
	    t = median([FIDSDISPLAY.XLIM point(1,1)]);
    ...
    set(findobj(allchild(handle),'tag','FIDSTYPE'),'value',find(events.num == FIDSDISPLAY.NEWFIDSTYPE)); %for loop fiducials, I guess?
...
function events = SetClosestEvent(events,t,y)
    % - sets/redraws the patch identified by sel,sel1,sel3 at the new
    % value t (which are set by FindClosestEvent)
...
% - if LoopFiducials is activated, set newtype to next type
          ...
          for q=1:max(FIDSDISPLAY.LEADGROUP),
              ...
              switch events.typelist(events.type(s)),
                  ...
                  case {2,3}, % interval fiducial/ fixed intereval fiducial
                      ...
                      events.handle(q,s) = patch('parent',events.axes,'Xdata',[v v v2 v2],'Ydata',[ydata ydata([2 1])],'FaceColor',events.colorlist{events.type(s)},'hittest','off','linewidth',events.linewidth{events.type(s)},'linestyle',events.linestyle{events.type(s)});
              end
              ch = [ch q];
          end
  ...
  if myScriptData.FIDSLOOPFIDS ==  1,
      FIDSDISPLAY.NEWFIDSTYPE = myScriptData.LOOP_ORDER(FIDSDISPLAY.NEWFIDSTYPE);  %TODO: I think this is bullshit
  end %the problem is, that events.next is set to strange default values..  Change them?
end   
...
%callback for KeyPress
    global myScriptData FIDSDISPLAY TS;
            ...
            if value == 0,   
                ...
                if (t>xwin(1))&(t<xwin(2))&(y>ywin(1))&(y<ywin(2)),
                    ...
                    switch FIDSDISPLAY.SELEVENTS
                        case 1,
                            TS{myScriptData.CURRENTTS}.timeframe = []; 
                        case 2,
                            TS{myScriptData.CURRENTTS}.templateframe = [];
                        case 3,
                            TS{myScriptData.CURRENTTS}.averageframe = [];
                    end
                end    
            end	
        case 93,
...
numgroups = length(myScriptData.GROUPLEADS);
ind = find(FIDSDISPLAY.EVENTS{2}.type == 3);  % find the T-wave events
if ~isempty(ind)
    for p=1:numgroups
        qstart(myScriptData.GROUPLEADS{p}) = FIDSDISPLAY.EVENTS{2}.value(p,ind(1),1);
        qend(myScriptData.GROUPLEADS{p})   = FIDSDISPLAY.EVENTS{2}.value(p,ind(1),2);
    end
end    
...
for p=1:numgroups
    for q=myScriptData.GROUPLEADS{p}
    ...
    for p=1:length(fids),
        ...
        if FIDSDISPLAY.MODE == 2,
         ...
         switch fids(p).type,
            ...
            case 16,
                mtype = 6; val1 = fids(p).value*isamplefreq; val2 = val1+TS{myScriptData.CURRENTTS}.baselinewidth/samplefreq;
            case 10,
        ...
        if (length(val1) == numchannels)&(length(val2) == numchannels)
            ...
            isgroup = 1;
            for q=1:length(myScriptData.GROUPLEADS),
                channels = myScriptData.GROUPLEADS{q};
...
function EventsToFids
    ...
    for p=1:length(FIDSDISPLAY.EVENTS{2}.type),
        ...
        for q=1:length(myScriptData.GROUPLEADS),
            v1(myScriptData.GROUPLEADS{q}) = FIDSDISPLAY.EVENTS{2}.value(q,p,1)*samplefreq;
            v2(myScriptData.GROUPLEADS{q}) = FIDSDISPLAY.EVENTS{2}.value(q,p,2)*samplefreq;
        end
        val1{end+1} = v1;
        val2{end+1} = v2;
