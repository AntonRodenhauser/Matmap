...
function InitFiducials(handle,mode)
    ...
    events.dt = 5/myScriptData.SAMPLEFREQ;
    events.value = [];
    events.type = [];
    events.handle = [];
...
function InitDisplayButtons(handle)
    ...
    if ismember(0, ismember(myScriptData.DISPLAYGROUPF, 1:length(group))) || (isempty(myScriptData.DISPLAYGROUPF))
...
% - if LoopFiducials is activated, set newtype to next type
  ...
  if myScriptData.FIDSLOOPFIDS ==  1
      FIDSDISPLAY.NEWFIDSTYPE = myScriptData.LOOP_ORDER(FIDSDISPLAY.NEWFIDSTYPE);  
  end
end
