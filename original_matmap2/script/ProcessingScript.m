...
function Init(filename)
    ...
    if exist(filename,'file'),
        ...
        handle = winProcessingScriptMenu;
    ...
    UpdateGroup;
    handle = winProcessingScriptSettings;
    UpdateFigure(handle);
    handle = winProcessingScriptMenu;
    UpdateFigure(handle);
            ...
            switch objtype,
                ...
                case {'listbox'},
                    ...
                    if ~isempty(cellarray), 
                        ...
                        set(obj,'string',{'NO ACQ FILES FOUND','',''},'max',3,'value',[],'enable','off');
                    end
...
function GetACQLabel
    ...
    d = dir('*.acq');
    if length(d) == 0, cd(olddir); return; end
...
function PreLoopScript
    ...
    for r=1:length(filenames),
        if ~isempty(strfind(filenames{r},'.acq')), index = [index r]; end
    end
    ...
    if ~isempty(index),
        ...
        if ~isempty(dashpos),
            ...
            errordlg('cannot decode the ACQ filenames');
            error('ERROR')
        end
...
function ProcessACQFile(inputfilename,inputfiledir)
    ...
    fieldstoload = {'SELFRAMES','AVERAGEMETHOD','AVERAGESTART','AVERAGEEND','AVERAGEFRAMES','TEMPLATEFRAMES'};
    ...
    if SCRIPT.DO_SLICE == 1,
        ...
        if SCRIPT.DO_SLICE_USER == 1,
            ...
            ExportUserSettings(inputfilename,index,{'SELFRAMES','AVERAGEMETHOD','AVERAGESTART','AVERAGEEND','AVERAGEFRAMES','TEMPLATEFRAMES','LEADINFO'});
        end
    ...
    if SCRIPT.DO_DETECT == 1,
        ...
        ExportUserSettings(inputfilename,index,{'SELFRAMES','AVERAGEMETHOD','AVERAGESTART','AVERAGEEND','AVERAGEFRAMES','TEMPLATEFRAMES','LEADINFO','FIDS','FIDSET','STARTFRAME'});
    end
