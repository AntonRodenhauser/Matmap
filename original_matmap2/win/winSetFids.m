function handle = winSetFids(varargin)
% FUNCTION handle = winSetFids(TSindex)
%
% DESCRIPTION
...
function SetTimeSeries(handle,TSindex)
    ...
    window.rms = sqrt(mean((TS{TSindex}.potvals).^2));
    ...
    set(handle,'name',sprintf('winSetFids:%s',TS{TSindex}.filename));
    Update(handle);
...
function handle = Init(TSindex)      
    ...
    window.zoombutton = guiCreateButton(handle,'Zoom','winSetFids(''ZoomOnOff'',gcf)',[X(1) Y(end-2) X(2) Y(end-1)]);
    ...
    window.fidtypebutton = guiCreatePopupMenu(handle,window.fidtypes,'winSetFids(''SetFidType'',gcf)',[X(2) Y(end-2) X(3) Y(end-1)]);
    ...
    window.events.newtypecolor = window.fidcolors{1};
    window.events.newtypenumber = 0;
    
    set(handle,'UserData',window); % Set these things in the userdata
    % so when the window is destroyed so is its data

    set(handle,'KeyPressFcn','winSetFids(''KeyPressed'',gcf);', ...
	   	'WindowButtonUpFcn','winSetFids(''ButtonUp'',gcf);', ...
   		'WindowButtonDownFcn','winSetFids(''ButtonDown'',gcf);', ...
                'WindowButtonMotionFcn','winSetFids(''ButtonMotion'',gcf);');
...
function Update(handle)
    ...
    for p = 1:length(window.events.Index),
     	Evt = window.events;
        Pos = Evt.Events(Evt.Index(p));
	window.events.Lines(p) = line('parent',window.axes,'XData',[Pos Pos],'YData',window.ylim,'EraseMode','xor','Color',Evt.Color{Evt.Index(p)});
    end
...
function Events = AddEvent(Events,T)
    ...
    TS{Events.TSindex}.fids(end).fidset = 0;
