function handle = winSetFids2(varargin)
% FUNCTION handle = winSetFids2(TSindex)
%
% DESCRIPTION
...
function SetTimeSeries(handle,TSindex)
    ...
    set(handle,'name',sprintf('winSetFids2:%s',TS{TSindex}.filename));
    Update(handle);
...
function handle = Init(TSindex)      
    ...
    window.zoombutton = guiCreateButton(handle,'Zoom','winSetFid2s(''ZoomOnOff'',gcf)',[X(1) Y(end-2) X(2) Y(end-1)]);
   ...
   % window.saveallbutton = guiCreateButton(handle,'Save All','winSetFids(''SaveAll'',gcf)',[X(6) Y(end-1) X(7) Y(end)]);
    windw.apply = guiCreateButton(handle,'Apply','winSetFids2(''Apply'',gcf)',[X(6) Y(end-2) X(7) Y(end-1)]); 
    ...
    window.fidtypebutton = guiCreatePopupMenu(handle,window.fidtypes,'winSetFids2(''SetFidType'',gcf)',[X(2) Y(end-2) X(3) Y(end-1)],3);
   ...
   % window.prevTS = guiCreateButton(handle,'Prev TS','winSetFids(''PrevTS'',gcf)',[X(1) Y(end-1) X(2) Y(end)]);
    ...
    set(handle,'KeyPressFcn','winSetFids2(''KeyPressed'',gcf);', ...
	   	'WindowButtonUpFcn','winSetFids2(''ButtonUp'',gcf);', ...
   		'WindowButtonDownFcn','winSetFids2(''ButtonDown'',gcf);', ...
                'WindowButtonMotionFcn','winSetFids2(''ButtonMotion'',gcf);');
