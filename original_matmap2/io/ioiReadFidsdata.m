function TS = ioiReadFidsdata(varargin)
% FUNCTION TS = ioiReadFidsdata(TS,filenames,options)
%
% DESCRIPTION
...
TS = varargin{1};
files = ioiInputParameters(varargin(2:end));
...
for r=1:numts,
    ...
    tsdfkey = utilFileParts(TS{r}.filename); 		% remove pathname and number extension
							% key is only the filename
