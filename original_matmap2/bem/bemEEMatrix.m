...
% SEE ALSO bemEJMatrix, bemJEMatrix, bemJJMatrix
    ...
    %   model.surface{}.facmaskp AND model.surface{}.facmaskj
    ...
    rowlen = 0;
    for p = surf1,
        rowlen = rowlen + size(model.surface{p}.pts,2);
    end
    ...
    for  p = 1:length(surf1),
        rows{p} = rowstart + [1:size(model.surface{surf1(p)}.pts,2)];
    ...
    collen = 0;
    for p = surf2,
        collen = collen + size(model.surface{p}.pts,2);
    end
    ...
    for p = 1:length(surf2),
        cols{p} = colstart + [1:size(model.surface{surf2(p)}.pts,2)];
...
%    fprintf(1,'Computing geometrical data : %02d %%',0);
    ...
    if isfield(model.surface{surf2},'facmaskp'),
        facmaskp = model.surface{surf2}.facmaskp;
        Tri = Tri(:,facmaskp);
    end    
