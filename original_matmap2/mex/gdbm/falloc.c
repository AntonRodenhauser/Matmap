...
_gdbm_alloc (dbf, num_bytes)
     ...
     int num_bytes;
{
  my_int64_t file_adr;		/* The address of the block. */
     ...
     gdbm_file_info *dbf;
{
  ...
  my_int64_t file_pos;		/* For use with the lseek system call. */
  avail_elem new_el;
  avail_block *new_blk;
  int index;
...
/* PATCH: replacement of read() => patch_readavailblock() */
  num_bytes = patch_readavailblock (dbf->desc, new_blk, new_el.av_size);
/* END PATCH */
...
/* PATCH: replacement of write() => patch_writeavailblock() */
  num_bytes = patch_writeavailblock (dbf->desc, temp, av_size);
/* END PATCH */  
  if (num_bytes != av_size) 
  {
  printf("num_bytes =%d and av_size=%d\n",num_bytes,av_size);
