...
_gdbm_get_bucket (dbf, dir_index)
     ...
     int dir_index;
{
  my_int64_t bucket_adr;	/* The address of the correct hash bucket.  */
  int   num_bytes;	/* The number of bytes read. */
  my_int64_t	file_pos;	/* The return address for lseek. */
...
/* PATCH: replacement of read() => patch_readhahsbucket() */
      num_bytes = patch_readhashbucket (dbf->desc, dbf->bucket, dbf->header->bucket_size);
     ...
     int next_insert;
{
  ...
  int	       cache_1;
  my_int64_t        adr_0;		/* File address of the new bucket 0. */
  my_int64_t        adr_1;		/* File address of the new bucket 1. */
  avail_elem   old_bucket;	/* Avail Struct for the old bucket. */

  my_int64_t        dir_start0;	/* Used in updating the directory. */
  my_int64_t        dir_start1;
  my_int64_t        dir_end;

  my_int64_t       *new_dir;		/* Pointer to the new directory. */
  my_int64_t        dir_adr; 	/* Address of the new directory. */
  int          dir_size;	/* Size of the new directory. */
  my_int64_t        old_adr[31]; 	/* Address of the old directories. */
  ...
  while (dbf->bucket->count == dbf->header->bucket_elems)
      ...
      if (dbf->header->dir_bits == dbf->bucket->bucket_bits)
	{
	  ...
	  new_dir  = (my_int64_t *) malloc (dir_size);
	  if (new_dir == NULL) _gdbm_fatal (dbf, "malloc error");
	  for (index = 0;
	  	index < dbf->header->dir_size/sizeof (my_int64_t); index++)
	    {
     ...
     cache_elem *ca_entry;
{
  ...
  my_int64_t file_pos;	/* The return value for lseek. */
...
/* PATCH: replacement of write() => patch_writehashbucket() */
  num_bytes = patch_writehashbucket (dbf->desc, ca_entry->ca_bucket, dbf->header->bucket_size);
  if (num_bytes != dbf->header->bucket_size)
