function TSmapindex = fidsIntST(TSindices)
% FUNCTION TSmapindex = fidsIntST(TSindices)
...
% Get starting and ending from fiducials 
 ...
 if isnumeric(TSindices),
    ...
    for p = 1:length(TSindices),
        ...
        frameend = framestart + (3/8)*(frameend-framestart);
 
        TSmapindex = fidsIntegralMap(TSmapindex,TSindices(p),framestart(:,1),frameend(:,1));
 
    end
    global TS;
    TS{TSmapindex}.newfileext = '_int_st';
    TS{TSmapindex}.audit = ['|ST integration ' TS{TSmapindex}.audit ];
    TS{TSmapindex}.label = 'ST Integralmap';
end
...
if iscell(TSindices),
    ...
    TSmapindex.newfileext = '_int_st';
    TSmapindex.audit = ['|ST integration ' TS{TSmapindex}.audit ];
    TSmapindex.label = 'ST Integralmap';
end    
