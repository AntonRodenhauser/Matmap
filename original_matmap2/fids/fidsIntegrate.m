function TSindexmap = fidsIntegrate(TSindices,startframe,endframe)
% FUNCTION TSindexmap = fidsIntegrate(TSindices,startframe,endframe)
...
% UPDATE JULY 2003
% Function accepts direct data input as well
    ...
    if (length(starttime) > 1)|(length(endtime) > 1),					% Check the dimensions of the fiducials
        ...
        if (length(starttime) ~= numleads)|(length(endtime)~=numleads), 
            ...
            for q=1:numleads,
                ...
                TS{TSindexmap}.potvals(q,intmapnum) = sum(TS{p}.potvals(q,index),2); % the actual integration
            end
        end
    else
        index = find((fm >= round(starttime))&(fm <=round(endtime)));
        TS{TSindexmap}.potvals(1:numleads,intmapnum) = sum(TS{p}.potvals(:,index),2); % the actual integration
    end    
